...
ssh-keygen
cat ~/.ssh/id_rsa.pub
# add to https://bitbucket.org/binaricore/ashbournewm2/admin/access-keys/ as a new key
...
git clone git@bitbucket.org:binaricore/ashbournewm2
cd ashbournewm2/ && git config core.filemode false && cd
mkdir ssl-for-origin/ && cd ssl-for-origin && sudo openssl req -x509 -nodes -days 3650 -newkey rsa:4096 -keyout ashbournewm2.key -out ashbournewm2.crt && cd


# In project directory (cd ~/wordpress/ashbournewm2) run deploy bash script sh ./scripts/deploy_dest.sh && sh ./scripts/restart_dest.sh
...
#Deploy ashbournewm2.com.key & ashbournewm2.com.crt for root user:
scp -r /home/${LOCAL_USER}/ssl-keys/ashbournewm2/ssl-for-origin  root@{DEST_SERVER_IP}:/home/proftit/
#OR for AMZ ec2-user
scp -i .ssh/ashbournewm2 -r /home/${LOCAL_USER}/ssl-keys/ashbournewm2/ssl-for-origin ec2-user@{DEST_SERVER_IP}:~/
