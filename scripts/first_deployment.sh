...
ssh-keygen
cat ~/.ssh/id_rsa.pub
# add to https://bitbucket.org/binaricore/ashbournewm/admin/access-keys/ as a new key
...
git clone git@bitbucket.org:binaricore/ashbournewm
cd ashbournewm/ && git config core.filemode false && cd
mkdir ssl-for-origin/ && cd ssl-for-origin && sudo openssl req -x509 -nodes -days 3650 -newkey rsa:4096 -keyout ashbournewm.key -out ashbournewm.crt && cd


# In project directory (cd ~/wordpress/ashbournewm) run deploy bash script sh ./scripts/deploy_dest.sh && sh ./scripts/restart_dest.sh
...
#Deploy ashbournewm.com.key & ashbournewm.com.crt for root user:
scp -r /home/${LOCAL_USER}/ssl-keys/ashbournewm/ssl-for-origin  root@{DEST_SERVER_IP}:/home/proftit/
#OR for AMZ ec2-user
scp -i .ssh/ashbournewm -r /home/${LOCAL_USER}/ssl-keys/ashbournewm/ssl-for-origin ec2-user@{DEST_SERVER_IP}:~/
